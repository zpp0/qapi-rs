...
use tokio::io::{AsyncBufReadExt, AsyncRead, AsyncReadExt, AsyncWrite, AsyncWriteExt, ReadHalf, WriteHalf};
use futures::{Future, TryFutureExt, Sink, Stream, future, try_join};
...
type QapiStreamLines<S> = Lines<S>;
...
#[cfg(any(feature = "qapi-qmp", feature = "qapi-qga"))]
impl<R: AsyncBufReadExt + Unpin> QapiEvents<R> {
    ...
    async fn process_message(&mut self) -> io::Result<QapiEventsMessage> {
        let msg = match self.lines.next_line().await? {
            #[cfg(feature = "qapi-qmp")]
    ...
    pub fn into_stream(self) -> impl Stream<Item=io::Result<qmp::Event>> + FusedStream {
        unfold(self, move |mut s| {
            async move { s.next_event().await.invert().map(|r| (r, s)) }
        })
    }
...
impl<S: AsyncRead + AsyncWrite> QapiStream<WriteHalf<S>> {
    pub async fn open_tokio(stream: S) -> io::Result<(qmp::QapiCapabilities, Self, QapiEvents<BufReader<ReadHalf<S>>>)> {
...
#[cfg(feature = "qapi-qmp")]
impl<S: AsyncRead + AsyncWrite> QapiStream<WriteHalf<S>> {
    pub async fn open(stream: S) -> io::Result<(qmp::QapiCapabilities, Self, QapiEvents<BufReader<ReadHalf<S>>>)> {
...
impl<S: tokio_io::AsyncRead + tokio_io::AsyncWrite> QapiStream<WriteHalf<Compat01As03<S>>> {
    pub async fn open_tokio_qga(stream: S) -> io::Result<(Self, impl Future<Output=()>)> {
        Self::open_qga(Compat01As03::new(stream)).await
    }
}
...
impl<W: AsyncWrite + Unpin> QapiStream<W> {
    pub async fn open_split<R: AsyncRead + Unpin>(read: R, write: W) -> io::Result<(qmp::QapiCapabilities, Self, QapiEvents<BufReader<R>>)> {
        let read = BufReader::new(read);
        let mut lines = read.lines();

        let greeting = lines.next_line().await?.unwrap();
